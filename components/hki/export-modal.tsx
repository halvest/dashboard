...
export function ExportModal({ isOpen, onClose, formOptions }: ExportModalProps) {
      ...
      case 'year':
        return (
          <Select value={filterValue} onValueChange={setFilterValue}>
            ...
            <SelectContent>
              {tahunOptions.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
            </SelectContent>
          </Select>
        );
      case 'pengusul':
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="py-4 space-y-6">
          <div className="space-y-3">
            ...
            <RadioGroup
              value={filterBy}
              onValueChange={(v: string) => handleFilterTypeChange(v as FilterType)}
          ...
          <div className="space-y-3">
            <Label htmlFor="filter-value" className="font-semibold">2. Pilih Nilai Filter</Label>
            {renderFilterInput()}
          </div>
          
          <div className="space-y-3">
            <Label className="font-semibold">3. Pilih Format File</Label>
            <RadioGroup defaultValue="xlsx" value={format} onValueChange={(v: string) => setFormat(v as ExportFormat)} className="flex items-center space-x-4">
