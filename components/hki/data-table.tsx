...
export const getStatusStyle = (statusName?: string) =>
  STATUS_STYLES[statusName as KnownStatus] || STATUS_STYLES['Default']
const DEFAULTS = {
  page: 1,
  pageSize: 10,
  sortBy: 'created_at',
...
const InteractiveExportModal = ({
  ...
  const activeFiltersSummary = useMemo(() => {
    ...
    if (filters.statusId) {
      const status = formOptions.statusOptions.find(
        (s: any) => s.id_status.toString() === filters.statusId
      )
      if (status) summary.push(`Status: ${status.nama_status}`)
    }
    if (filters.pengusulId) {
      const pengusul = formOptions.pengusulOptions.find(
        (p: any) => p.value === filters.pengusulId
      )
      if (pengusul) summary.push(`Pengusul: ${pengusul.label}`)
    }
    if (filters.jenisId) {
      const jenis = formOptions.jenisOptions.find(
        (j: any) => j.id_jenis_hki.toString() === filters.jenisId
      )
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !isExporting && onClose()}>
      <DialogContent className="sm:max-w-md">
        <SimpleDialogHeader>
          ...
          <DialogDescription>
            Anda akan mengunduh semua data yang cocok dengan filter yang sedang
            aktif di tabel.
          </DialogDescription>
        </SimpleDialogHeader>
...
const DataTableToolbar = memo(
  ({
    ...
    const selectedJenisLabel = formOptions.jenisOptions.find(
      (o: any) => o.id_jenis_hki.toString() === filters.jenisId
    )?.nama_jenis_hki
    const selectedStatusLabel = formOptions.statusOptions.find(
      (o: any) => o.id_status.toString() === filters.statusId
    )?.nama_status
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {formOptions.jenisOptions.map((opt: JenisHKI) => (
                    <SelectItem
              ...
              >
                ...
                <SelectContent>
                  ...
                  {formOptions.statusOptions.map((opt: StatusHKI) => (
                    <SelectItem
                      key={opt.id_status}
            ...
            {shouldShowBottomBar && (
              <div className="flex flex-wrap items-center gap-4 pt-2">
                ...
                {activeFiltersCount > 0 && (
                  ...
                  >
                    <X className="h-4 w-4" /> Bersihkan Semua Filter (
                    {activeFiltersCount})
                  </Button>
                )}
              </div>
  ...
  ({
    ...
    const handleDownloadPDF = useCallback(() => {
      ...
      fetch(`/api/hki/${entry.id_hki}/signed-url`)
        .then((res) => {
          if (!res.ok) throw new Error('Gagal mendapatkan URL.')
          return res.json()
        })
        .then(({ signedUrl }) => {
        ...
        .catch((error) => {
          toast.error(error.message || 'Gagal mengunduh.', { id: toastId })
        })
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild disabled={isPending}>
          <Button
            variant="outline"
            className={cn(
              'h-auto px-2 py-1 text-sm font-medium disabled:opacity-100 w-full justify-start gap-2 disabled:cursor-wait',
    ...
    return (
      <>
        <TableRow
          ...
          className={cn(
            'dark:border-slate-800 transition-colors duration-1000 ease-out',
            'hidden md:table-row',
        ...
        >
          ...
          <TableCell className="text-center font-mono text-sm text-muted-foreground py-2 px-4 align-top">
          ...
          <TableCell className="text-sm text-muted-foreground break-words py-2 px-4 align-top">
            {entry.pengusul?.nama_opd || '-'}
          </TableCell>
          <TableCell className="text-center font-mono text-sm text-foreground py-2 px-4 align-top">
...
const DataTablePagination = memo(
  ({
    ...
    const setPage = (page: number) =>
      setPagination((p: any) => ({ ...p, page: clamp(page, 1, totalPages) }))
  ...
  ({
    ...
    return (
      <TableHead
        ...
        className={cn(
          'cursor-pointer select-none group whitespace-nowrap font-medium px-4',
          className
        )}
        role="columnheader"
        aria-sort={
...
export function DataTable({
  ...
  const [deleteAlert, setDeleteAlert] = useState({
    open: false,
    entry: undefined as HKIEntry | undefined,
    isBulk: false,
  })
  ...
  const handleStatusUpdate = useCallback(
    async (entryId: number, newStatusId: number) => {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border dark:border-slate-800 bg-white dark:bg-slate-950">
        <div
          ...
          className="overflow-x-auto horizontal-scroll-indicator"
        >
          <Table>
            <TableHeader className="bg-slate-50 dark:bg-slate-900/50 hidden md:table-header-group">
              <TableRow className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                  ...
                  <TableHead className="w-12 sticky left-0 bg-inherit z-20 px-4 py-2 border-r dark:border-slate-800">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked)
                          data.forEach((row) => newRows.add(row.id_hki))
                ...
                <TableHead className="w-[60px] text-center font-medium py-2 px-4">
                  No
                </TableHead>
                <SortableHeader
                  columnId="nama_hki"
                ...
                <TableHead className="w-56 font-medium py-2 px-4">
                  Pemohon
                </TableHead>
                <TableHead className="w-48 font-medium py-2 px-4">
                  Jenis & Kelas
                </TableHead>
                <TableHead className="w-56 font-medium py-2 px-4">
                  Pengusul (OPD)
                ...
                <TableHead className="w-80 font-medium py-2 px-4">
                  Keterangan
                </TableHead>
                <TableHead className="w-48 font-medium py-2 px-4">
                  Status
                </TableHead>
                <TableHead className="w-24 text-right sticky right-0 bg-inherit z-20 px-4 py-2 border-l dark:border-slate-800 font-medium">
                  Aksi
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                Array.from({ length: tableState.pagination.pageSize }).map(
                  (_, i) => (
                    <TableRow
                      key={`skeleton-${i}`}
                      ...
                      {Array.from({ length: columnsCount }).map((__, j) => (
                        <TableCell
                          ...
                          className={cn(
                            ...
                            showCheckboxColumn &&
                              j === 0 &&
                              'sticky left-0 bg-white dark:bg-slate-950 border-r dark:border-slate-800',
                            j === columnsCount - 1 &&
                              'sticky right-0 bg-white dark:bg-slate-950 border-l dark:border-slate-800'
                          )}
                        >
