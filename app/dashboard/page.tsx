...
async function StatsCards() {
  try {
    ...
    if (statusError)
      throw new Error(`Gagal fetch status: ${statusError.message}`)

    const statusMap = statuses.reduce(
      (acc, status) => ({ ...acc, [status.nama_status]: status.id_status }),
    ...
    const [totalRes, diterimaRes, diprosesRes, ditolakRes] = await Promise.all([
      ...
      supabase
        ...
        .in(
          'id_status',
          [statusMap['Diterima'], statusMap['Didaftar']].filter(Boolean)
        ),
      supabase
        .from('hki')
        .select('*', { count: 'exact', head: true })
        .eq('id_status', statusMap['Dalam Proses']),
      supabase
        .from('hki')
        .select('*', { count: 'exact', head: true })
        .eq('id_status', statusMap['Ditolak']),
    ])
...
async function RecentActivity() {
  try {
    ...
    const { data: recentEntries, error } = await supabase
      .from('hki')
      .select(
        `id_hki, nama_hki, created_at, pemohon(nama_pemohon), status_hki(nama_status)`
      )
