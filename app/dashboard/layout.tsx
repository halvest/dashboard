...
type AuthResult = {
  user: any;
  errorType: null | 'unauthenticated' | 'unauthorized' | 'profile_not_found' | 'database_error' | 'unexpected_error';
  errorMessage?: string;
};
...
async function protectAdminRoute(): Promise<AuthResult> {
  try {
    ...
    if (profileError) {
      ...
      return { user: null, errorType: 'database_error', errorMessage: profileError.message };
    }

    if (!profile) {
      console.error(`Integritas data error: Tidak ditemukan profil untuk user ID: ${user.id}`);
    ...
    console.error("Terjadi error tak terduga di fungsi protectAdminRoute:", error);
    return { user: null, errorType: 'unexpected_error', errorMessage: error instanceof Error ? error.message : "Terjadi kesalahan tidak diketahui" };
  }
}
...
// ============================================================================
const ErrorDisplay = ({ icon: Icon, title, description, details }: { icon: React.ElementType, title: string, description: string, details?: string }) => (
...
// ============================================================================
export default async function HKILayout({ children }: { children: React.ReactNode }) {
  ...
  if (errorType) {
    ...
    switch (errorType) {
      case 'unauthorized':
        errorContent = <ErrorDisplay icon={ShieldOff} title="Akses Ditolak" description="Anda tidak memiliki izin untuk mengakses halaman ini. Silakan hubungi administrator jika Anda merasa ini adalah kesalahan." />;
        break;
      case 'database_error':
        errorContent = <ErrorDisplay icon={ServerCrash} title="Kesalahan Database" description="Gagal terhubung atau mengambil data dari database." details={errorMessage} />;
        break;
      case 'profile_not_found':
        errorContent = <ErrorDisplay icon={ServerCrash} title="Profil Tidak Ditemukan" description="Data profil Anda tidak ditemukan di sistem. Ini adalah kesalahan integritas data." />;
        break;
      default:
        errorContent = <ErrorDisplay icon={ServerCrash} title="Terjadi Kesalahan Tak Terduga" description="Sistem mengalami masalah yang tidak diharapkan." details={errorMessage} />;
    }
