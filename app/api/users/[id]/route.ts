...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: { user: requester } } = await supabaseSession.auth.getUser();
  if (!requester) {
    return NextResponse.json({ message: 'Akses ditolak: Tidak terautentikasi' }, { status: 401 });
  }
  const { data: profile } = await supabaseSession.from('profiles').select('role').eq('id', requester.id).single();
  if (profile?.role !== 'admin') {
    return NextResponse.json({ message: 'Hanya admin yang dapat mengubah data' }, { status: 403 });
  }
  ...
  try {
    ...
    if (Object.keys(profileUpdateData).length > 0) {
      const { error: profileError } = await supabaseAdmin.from('profiles').update(profileUpdateData).eq('id', userIdToUpdate);
    ...
    return NextResponse.json({ message: `Gagal memperbarui pengguna: ${error.message}` }, { status: 500 });
  }
}
